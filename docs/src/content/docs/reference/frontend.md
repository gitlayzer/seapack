...
description: Complete reference for the SeaPack BuildKit frontend
---

Complete reference documentation for the SeaPack BuildKit frontend. For a full example of how to use the frontend for a production build, see the [running SeaPack in production](../guides/running-seapack-in-production) guide.

## Expected

To use the SeaPack BuildKit frontend, you must provide a build plan file (e.g.
`seapack-plan.json`) generated by the `seapack prepare` command. This file
...
1. Pass secret names to `seapack prepare` (so they are included in the build
   plan):
