...
SeaPack supports installing additional versioned packages from
[Mise](https://mise.jdx.dev/), or packages from Apt.

## Mise

You can set the `SEAPACK_PACKAGES` environment variable to install additional
packages from Mise.

For example, this installs the latest versions of Node and Bun, and Python 3.10.

```bash
SEAPACK_PACKAGES="node bun@latest python@3.10"
```
...
You can set the `SEAPACK_BUILD_APT_PACKAGES` and `SEAPACK_DEPLOY_APT_PACKAGES`
...
```bash
SEAPACK_BUILD_APT_PACKAGES="build-essential"
SEAPACK_DEPLOY_APT_PACKAGES="ffmpeg"
```
