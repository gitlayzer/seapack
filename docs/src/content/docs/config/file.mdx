...
description: Learn about the seapack.json configuration file format and options
---

SeaPack will look for a `seapack.json` file in the root of the directory being
built. You can override this by setting the `SEAPACK_CONFIG_FILE` environment
...
```json
{
  "$schema": "https://seapack.sealos.io/schema.json",
  "steps": {
    "install": {
...
"deploy": {
  "base": {
    "image": "ghcr.io/gitlayzer/seapack-runtime:mise-2026.2.22"
  },
  "inputs": [
    {
      "step": "packages:mise",
...
The schema for the config file is available at https://seapack.sealos.io/schema.json. Add
it to your `seapack.json` to get autocomplete and validation in your editor.
