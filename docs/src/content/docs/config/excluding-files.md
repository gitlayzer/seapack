...
SeaPack supports the standard `.dockerignore` file syntax to exclude files from your build. For a complete reference on the syntax, please refer to the [official Docker documentation](https://docs.docker.com/build/building/context/#dockerignore-files).

## Default Behavior

If no `.dockerignore` file is present in your project root, SeaPack defaults to including **all files** in the build context.
