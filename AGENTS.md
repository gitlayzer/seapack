...
Sealos-oriented application builder that automatically analyzes source code and
turns supported workloads into container images. It's built on BuildKit with
support for Node, Python, Go, Java, and Deno.
...
- Do not use `bin/seapack` instead use `mise run cli` (which is the development build of `seapack`)
  - Therefore do not run `mise build`, we don't need a `seapack` binary for local testing
...
* They represent example projects that would be built using the `seapack` CLI
* On CI, they are built and run to make sure `seapack` properly builds *and* runs the project
