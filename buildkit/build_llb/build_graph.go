...
const seaPackAlpineImage = "ghcr.io/gitlayzer/seapack-alpine:latest"
...
func NewBuildGraph(plan *plan.BuildPlan, localState *llb.State, cacheStore *BuildKitCacheStore, secretsHash string, platform *specs.Platform, githubToken string) (*BuildGraph, error) {
	...
	if secretsHash != "" {
		st := llb.Scratch().File(llb.Mkfile("/secrets-hash", 0644, []byte(secretsHash)), llb.WithCustomName("[seapack] secrets hash"))
		secretsFile = &st
	}
	usedSecretsBase := llb.Image(seaPackAlpineImage, llb.WithCustomName("[seapack] loading secrets"))
...
func isCacheDisabled(key string) bool {
	...
	disabled := os.Getenv("SEAPACK_DISABLE_CACHES")
